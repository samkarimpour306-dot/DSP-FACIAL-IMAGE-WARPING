...
# Load .env before any module uses os.getenv("REPLICATE_API_TOKEN")
try:
...
_PY_ROOT = Path(sys.executable).resolve().parent
_TCL_DIR = _PY_ROOT / "tcl"
if _TCL_DIR.exists():
...
class _SliderRow(tk.Frame):
    ...
    def __init__(self, parent, label, key, vmin, vmax, init, color, sv: dict,
        ...
        self._scale = tk.Scale(
            ...
            resolution=1.0 if self._is_int else 0.01,
        )
    ...
    def _fmt(self, v: float) -> str:
        return f"{v:.0f}" if self._is_int else f"{v:.2f}"
...
class PhotoixApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # slider values
        self._sv           = dict(
            smile=0.0, eyebrow=0.0, lip=0.0, slim=0.0, sigma=50.0,
            glasses_x=0.0, glasses_y=0.0, glasses_scale=1.0, glasses_rotate=0.0,
            beard_x=3.33, beard_y=56.67, beard_scale=1.42,
        )
        ...
        self.sunglasses_on       = tk.BooleanVar(value=False)
        self.sunglasses_style    = tk.StringVar(value="classic")
        self.beard_on            = tk.BooleanVar(value=False)
        self.beard_color         = tk.StringVar(value="black")
    ...
    def _setup_window(self):
        self.root.title("Photix  ·  Facial Processing Studio  ·  Sunglasses")
    ...
    def _fill_sidebar(self, p: tk.Frame):
        ...
        _Btn(p, "Add Sunglasses", self.add_sunglasses,
             accent=CYAN, fg=BG0).pack(fill=tk.X, padx=8, pady=(0, 2))
        ...
        # ── warp ──────────────────────────────────────────────────────────
        self._build_overlay_controls(p)
        self._build_beard_controls(p)
    ...
    def _build_overlay_controls(self, p: tk.Frame):
        _sect(p, "SUNGLASSES")
        sg_row = tk.Frame(p, bg=BG1)
        sg_row.pack(fill=tk.X, padx=8, pady=(2, 0))
        tk.Checkbutton(
        ...
        style_frame = tk.Frame(p, bg=BG1)
        style_frame.pack(fill=tk.X, padx=18, pady=(1, 4))
        for txt, val in [("Classic", "classic"), ("Aviator", "aviator"), ("Round", "round"), ("Glasses", "glasses"), ("PNG", "png")]:
            ttk.Radiobutton(style_frame, text=txt,
                            variable=self.sunglasses_style, value=val).pack(
                side=tk.LEFT, padx=(0, 6))

        for lbl, key, vmin, vmax, init, color in [
            ("Move X", "glasses_x", -220.0, 220.0, 0.0, CYAN),
            ("Move Y", "glasses_y", -220.0, 220.0, 0.0, CYAN),
            ("Size",   "glasses_scale", 0.50, 2.00, 1.0, GREEN),
            ("Rotate", "glasses_rotate", -35.0, 35.0, 0.0, AMBER),
        ]:
            row = _SliderRow(p, lbl, key, vmin, vmax, init, color, self._sv)
            row.pack(fill=tk.X, padx=8, pady=1)
            self._slider_rows.append(row)
        _sep(p)

    def _build_beard_controls(self, p: tk.Frame):
        _sect(p, "BEARD")
        br_row = tk.Frame(p, bg=BG1)
        br_row.pack(fill=tk.X, padx=8, pady=(2, 0))
        tk.Checkbutton(
        ...
        color_frame.pack(fill=tk.X, padx=18, pady=(1, 4))
        for txt, val in [("Black", "black"), ("Brown", "brown"), ("Blonde", "blonde")]:
            ttk.Radiobutton(color_frame, text=txt,
                            variable=self.beard_color, value=val).pack(
                side=tk.LEFT, padx=(0, 6))

        for lbl, key, vmin, vmax, init, color in [
            ("Move X",  "beard_x",     -150.0, 150.0, 3.33, CYAN),
            ("Move Y",  "beard_y",     -150.0, 150.0, 56.67, CYAN),
            ("Size",    "beard_scale",    0.5,   2.0,  1.42, GREEN),
        ]:
            row = _SliderRow(p, lbl, key, vmin, vmax, init, color, self._sv)
            row.pack(fill=tk.X, padx=8, pady=1)
            self._slider_rows.append(row)
    ...
    def apply_processing(self):
        ...
        def _run():
            try:
                ...
                if self.sunglasses_on.get():
                    if lm is None:
                        raise RuntimeError("Sunglasses need a detected face. Load a clear face image first.")
                    engine = FaceOverlayEngine(lm, img)
                    sg_style = self.sunglasses_style.get()
                    if sg_style == "png":
                        effect_name = "sunglasses_png"
                        effect_params = {
                            "sunglasses_png": {
                                "offset_x": self._sv["glasses_x"],
                                "offset_y": self._sv["glasses_y"],
                                "scale":    self._sv["glasses_scale"],
                                "rotation": self._sv["glasses_rotate"],
                            }
                        }
                    else:
                        effect_name = "sunglasses"
                        effect_params = {
                            "sunglasses": {
                                "style":    sg_style,
                                "offset_x": self._sv["glasses_x"],
                    ...
                    result = engine.apply([effect_name], effect_params)
                    img = result.final_image

                if self.beard_on.get():
                    if lm is None:
                        raise RuntimeError("Beard needs a detected face. Load a clear face image first.")
                    engine = FaceOverlayEngine(lm, img)
                    result = engine.apply(
                        ["beard"],
                        {"beard": {
                            "color":    self.beard_color.get(),
                            "offset_x": self._sv["beard_x"],
                            "offset_y": self._sv["beard_y"],
                            "scale":    self._sv["beard_scale"],
                        }},
                    )
                    img = result.final_image
