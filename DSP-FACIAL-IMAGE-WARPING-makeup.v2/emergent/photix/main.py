...
class PhotoixApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # makeup vars — shared between static-image and live/video pipelines.
        # Each layer has its own enabled flag, colour and intensity so the
        # user can freely mix (e.g. blue eyeshadow + red lipstick + blush).
        self.mk_eyeshadow_on    = tk.BooleanVar(value=False)
        self.mk_eyeshadow_color = tk.StringVar(value="brown")
        self.mk_eyeshadow_int   = tk.DoubleVar(value=0.55)
        self.mk_blush_on        = tk.BooleanVar(value=False)
        self.mk_blush_color     = tk.StringVar(value="pink")
        self.mk_blush_int       = tk.DoubleVar(value=0.30)
        self.mk_lipstick_on     = tk.BooleanVar(value=False)
        self.mk_lipstick_color  = tk.StringVar(value="red")
        self.mk_lipstick_int    = tk.DoubleVar(value=0.62)
        self.mk_eyeliner_on     = tk.BooleanVar(value=False)
        self.mk_eyeliner_color  = tk.StringVar(value="black")
        self.mk_eyeliner_int    = tk.DoubleVar(value=0.85)
        self.mk_mascara_on      = tk.BooleanVar(value=False)
        self.mk_mascara_color   = tk.StringVar(value="black")
        self.mk_mascara_int     = tk.DoubleVar(value=0.75)
    ...
    def _fill_sidebar(self, p: tk.Frame):
        ...
        # ── makeup ────────────────────────────────────────────────────────
        # Five independent layers, each with its own enable toggle, colour
        # combobox and intensity slider. Same vars feed both the static
        # photo pipeline (apply_processing) and the live/video pipeline.
        _sect(p, "MAKEUP")
        mk = tk.Frame(p, bg=BG1)
        mk.pack(fill=tk.X, padx=14, pady=(0, 4))

        def _mk_apply(_=None):
            self._apply_processing_if_image_loaded()

        def _mk_row(parent, label, on_var, color_var, color_values, int_var):
            row = tk.Frame(parent, bg=BG1)
            row.pack(fill=tk.X, pady=(4, 0))
            ttk.Checkbutton(row, text=label, variable=on_var,
                            command=_mk_apply).pack(anchor="w")
            box = ttk.Combobox(row, textvariable=color_var,
                               values=list(color_values),
                               state="readonly", width=12)
            box.pack(anchor="w", padx=(18, 0), pady=1)
            box.bind("<<ComboboxSelected>>", _mk_apply)
            sld = ttk.Scale(row, from_=0.0, to=1.0, variable=int_var,
                            orient="horizontal", length=140,
                            command=lambda _v: _mk_apply())
            sld.pack(anchor="w", padx=(18, 0), pady=(1, 0))

        _mk_row(mk, "Göz Farı (eyeshadow)", self.mk_eyeshadow_on,
                self.mk_eyeshadow_color,
                ("blue", "black", "red", "green", "orange", "brown", "glitter"),
                self.mk_eyeshadow_int)
        _mk_row(mk, "Allık (blush)", self.mk_blush_on,
                self.mk_blush_color, ("pink", "brown"), self.mk_blush_int)
        _mk_row(mk, "Ruj (lipstick)", self.mk_lipstick_on,
                self.mk_lipstick_color, ("red", "pink", "brown", "gloss"),
                self.mk_lipstick_int)
        _mk_row(mk, "Eyeliner", self.mk_eyeliner_on,
                self.mk_eyeliner_color, ("black", "brown", "blue", "green"),
                self.mk_eyeliner_int)
        _mk_row(mk, "Kirpik (mascara)", self.mk_mascara_on,
                self.mk_mascara_color, ("black", "brown"),
                self.mk_mascara_int)
        _sep(p)
    ...
    # ── MAKEUP CONFIG HELPER ─────────────────────────────────────────────────
    def _build_makeup_config(self) -> dict:
        """Pack the makeup Tk vars into the dict format apply_makeup expects.

        Returned dict is also reused by the live/video pipeline so the static
        photo preview and the live preview look identical at the same colours.
        """
        return {
            "eyeshadow": {"enabled": bool(self.mk_eyeshadow_on.get()),
                          "color":   str(self.mk_eyeshadow_color.get() or "brown"),
                          "intensity": float(self.mk_eyeshadow_int.get())},
            "blush":     {"enabled": bool(self.mk_blush_on.get()),
                          "color":   str(self.mk_blush_color.get() or "pink"),
                          "intensity": float(self.mk_blush_int.get())},
            "lipstick":  {"enabled": bool(self.mk_lipstick_on.get()),
                          "color":   str(self.mk_lipstick_color.get() or "red"),
                          "intensity": float(self.mk_lipstick_int.get())},
            "eyeliner":  {"enabled": bool(self.mk_eyeliner_on.get()),
                          "color":   str(self.mk_eyeliner_color.get() or "black"),
                          "intensity": float(self.mk_eyeliner_int.get())},
            "mascara":   {"enabled": bool(self.mk_mascara_on.get()),
                          "color":   str(self.mk_mascara_color.get() or "black"),
                          "intensity": float(self.mk_mascara_int.get())},
        }
    ...
    def apply_processing(self):
        ...
        def _run():
            try:
                ...
                # ── Apply makeup (eyeshadow / blush / lipstick / eyeliner /
                # mascara). Done last so it sits on top of beard/hair/glasses.
                mk_cfg = self._build_makeup_config()
                if lm is not None and any(v["enabled"] for v in mk_cfg.values()):
                    from effects.makeup import apply_makeup as _apply_makeup
                    img = _apply_makeup(img, lm, mk_cfg).final_image
    ...
    def _build_live_pipeline(self) -> LivePipeline:
        ...
        # ── Makeup — same vars feed both static and live preview ─────────
        pipe.makeup_eyeshadow_enabled = bool(self.mk_eyeshadow_on.get())
        pipe.makeup_eyeshadow_color   = str(self.mk_eyeshadow_color.get() or "brown")
        pipe.makeup_eyeshadow_intensity = float(self.mk_eyeshadow_int.get())
        pipe.makeup_blush_enabled     = bool(self.mk_blush_on.get())
        pipe.makeup_blush_color       = str(self.mk_blush_color.get() or "pink")
        pipe.makeup_blush_intensity   = float(self.mk_blush_int.get())
        pipe.makeup_lipstick_enabled  = bool(self.mk_lipstick_on.get())
        pipe.makeup_lipstick_color    = str(self.mk_lipstick_color.get() or "red")
        pipe.makeup_lipstick_intensity = float(self.mk_lipstick_int.get())
        pipe.makeup_eyeliner_enabled  = bool(self.mk_eyeliner_on.get())
        pipe.makeup_eyeliner_color    = str(self.mk_eyeliner_color.get() or "black")
        pipe.makeup_eyeliner_intensity = float(self.mk_eyeliner_int.get())
        pipe.makeup_mascara_enabled   = bool(self.mk_mascara_on.get())
        pipe.makeup_mascara_color     = str(self.mk_mascara_color.get() or "black")
        pipe.makeup_mascara_intensity = float(self.mk_mascara_int.get())
    ...
    def reset(self):
        ...
        # Reset makeup layers too
        for v in (self.mk_eyeshadow_on, self.mk_blush_on, self.mk_lipstick_on,
                  self.mk_eyeliner_on, self.mk_mascara_on):
            v.set(False)
        if self.current_view == "images":
            self._refresh_images()
