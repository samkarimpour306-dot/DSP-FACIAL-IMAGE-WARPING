...
> 
> **Now with virtual makeup** — pose-stable eyeshadow, blush, lipstick,
> eyeliner and mascara on photos, live webcam and video. See
> [`MAKEUP_README.md`](MAKEUP_README.md) for full details.
...
# 2. Run the application (desktop GUI)
python main.py

# Or: live webcam directly (no Tk shell)
python start_live.py            # empty pipeline, build with keyboard
python start_live_makeup.py     # opens with a full makeup look pre-applied

# Or: web UI (Gradio)
python ui.py                    # http://127.0.0.1:7860
```

In the desktop GUI the makeup controls live in the sidebar under **MAKEUP**
(five toggleable layers, each with a colour combobox and intensity slider).
In the webcam window use `e/E b/B r/R n/N k/K` to toggle / cycle colours
for eyeshadow, blush, lipstick, eyeliner and mascara.
