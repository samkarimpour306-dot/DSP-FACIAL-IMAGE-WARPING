...
class FaceOverlayEngine:
    ...
    def _effect_makeup(
        ...
        **layer_overrides,
        ...
        """Virtual makeup layer (eyeshadow, blush, lipstick, eyeliner, mascara).

        Accepts either a full ``config`` dict (layer -> settings) or individual
        layer keyword arguments, e.g.::

            engine.apply(["makeup"], {"makeup": {"config": {
                "lipstick": {"enabled": True, "color": "red"},
            }}})
        """
        from effects.makeup import apply_makeup  # local import avoids cycles

        merged = dict(config or {})
        merged.update(layer_overrides)
        result = apply_makeup(image, self._pixels, merged)
        return result.final_image, result.masks_used, result.dsp_metadata
