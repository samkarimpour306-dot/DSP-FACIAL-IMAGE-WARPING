...
class LivePipeline:
    ...
    # Virtual makeup — landmark-driven, pose-stable per frame.
    # Each layer is independently toggleable; colours are English keys
    # (Turkish synonyms are also accepted by the engine).
    makeup_eyeshadow_enabled: bool  = False
    makeup_eyeshadow_color:   str   = "brown"   # blue|black|red|green|orange|brown|glitter
    makeup_eyeshadow_intensity: float = 0.55

    makeup_blush_enabled:     bool  = False
    makeup_blush_color:       str   = "pink"    # pink|brown
    makeup_blush_intensity:   float = 0.30

    makeup_lipstick_enabled:  bool  = False
    makeup_lipstick_color:    str   = "red"     # red|pink|brown|gloss
    makeup_lipstick_intensity: float = 0.62

    makeup_eyeliner_enabled:  bool  = False
    makeup_eyeliner_color:    str   = "black"
    makeup_eyeliner_intensity: float = 0.85

    makeup_mascara_enabled:   bool  = False     # kirpik
    makeup_mascara_color:     str   = "black"
    makeup_mascara_intensity: float = 0.75
    ...
    def has_makeup(self) -> bool:
        return (self.makeup_eyeshadow_enabled
                or self.makeup_blush_enabled
                or self.makeup_lipstick_enabled
                or self.makeup_eyeliner_enabled
                or self.makeup_mascara_enabled)

    def makeup_config(self) -> dict[str, dict[str, Any]]:
        """Build the dict consumed by effects.makeup.apply_makeup."""
        return {
            "eyeshadow": {"enabled": self.makeup_eyeshadow_enabled,
                          "color":   self.makeup_eyeshadow_color,
                          "intensity": self.makeup_eyeshadow_intensity},
            "blush":     {"enabled": self.makeup_blush_enabled,
                          "color":   self.makeup_blush_color,
                          "intensity": self.makeup_blush_intensity},
            "lipstick":  {"enabled": self.makeup_lipstick_enabled,
                          "color":   self.makeup_lipstick_color,
                          "intensity": self.makeup_lipstick_intensity},
            "eyeliner":  {"enabled": self.makeup_eyeliner_enabled,
                          "color":   self.makeup_eyeliner_color,
                          "intensity": self.makeup_eyeliner_intensity},
            "mascara":   {"enabled": self.makeup_mascara_enabled,
                          "color":   self.makeup_mascara_color,
                          "intensity": self.makeup_mascara_intensity},
        }
    ...
    def label(self) -> str:
        ...
        if self.has_makeup():
            mk: list[str] = []
            if self.makeup_eyeshadow_enabled: mk.append(f"eye:{self.makeup_eyeshadow_color}")
            if self.makeup_blush_enabled:    mk.append(f"blush:{self.makeup_blush_color}")
            if self.makeup_lipstick_enabled: mk.append(f"lip:{self.makeup_lipstick_color}")
            if self.makeup_eyeliner_enabled: mk.append("liner")
            if self.makeup_mascara_enabled:  mk.append("mascara")
            parts.append("Makeup[" + ",".join(mk) + "]")
...
def apply_pipeline(
    ...
    try:
        ...
        # ── 5. Virtual makeup — pose-stable, recomputed every frame ────────
        # Always last so makeup sits on top of beard/hair/sunglasses if any.
        if landmarks is not None and pipeline.has_makeup():
            from effects.makeup import apply_makeup
            img = apply_makeup(img, landmarks, pipeline.makeup_config()).final_image
    except Exception:
        return frame_bgr
